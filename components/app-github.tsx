...
const Githubcontributions = () => {
  ...
  function filterCurrentYear(data: Activity[]): Activity[] {
    ...
    return data.filter(
      (item) =>
        item.date.startsWith(lastYear) || item.date.startsWith(thisYear),
    );
  }
  return isLoading ? (
    ...
    <TooltipProvider>
      <ContributionGraph
        data={filterCurrentYear(contribution)}
        blockMargin={4}
        blockSize={16}
        fontSize={14}
      >
        <ContributionGraphCalendar
          className="py-3 [&::-webkit-scrollbar]:h-1.5
      ...
      dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500"
        ...
        <ContributionGraphFooter>
          <ContributionGraphTotalCount>
            {({ totalCount, year }) => (
              <div className="flex items-center justify-end w-full py-2 gap-2">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      onClick={() =>
                  ...
                  <TooltipContent className="flex items-center gap-2">
                    <Github className="size-4" /> My Github
                  </TooltipContent>
                </Tooltip>
